...
class ServicesVoter extends Voter
{
    ...
    /**
     * Cette méthode vérifie si l'utilisateur a le droit d'effectuer une action dans le ServicesCrudController.
     *
     ...
     * @return bool Retourne true si l'utilisateur a le droit à l'action dans le ServicesCrudController, sinon false.
     */
    protected function voteOnAttribute($attribute, $subject, TokenInterface $token): bool
    {
        ...
        if ($crudController !== ServicesCrudController::class) {
            return true;
        }
    ...
    /**
     * Filtre l'accès aux action de modification, suppression et création d'un service uniquement pour l'administrateurs et de modification pour l'employé .
     *
    ...
    private function canExecuteAction($context): bool
  {
      ...
      // Les actions 'new'et 'delete' sont réservées aux administrateurs
      if (in_array($action, ['new', 'delete'])) {
          return $this->security->isGranted('ROLE_ADMIN');
      }
      // L' actions 'edit' est réservées à l'administrateurs et aux employés
